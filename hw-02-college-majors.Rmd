...
```{r}
  ...
  labs(title = "Distribution of Median Income (Binwidth = $5000)")
```
    
    ***5000 Binwidth is what I prefer considering it makes the data easier to interpret.***
...
***The distribution is skewed meaning that most majors have lower median incomes whilst a few have higher incomes.***
...
college_recent_grads %>%
  ...
  summarise(highest_median_income = max(median)) %>%
  arrange(desc(highest_median_income))
```
...
overall_median <- median(college_recent_grads$median)

college_recent_grads %>%
  filter(major_category %in% c("Engineering", "Computer Science", "Physical Sciences", "Mathematics & Statistics") & median <= overall_median) %>%
...
college_recent_grads$stem <- ifelse(college_recent_grads$major_category %in% c("Engineering", "Computer Science", "Physical Sciences", "Mathematics & Statistics"), "STEM", "Non-STEM")

ggplot(college_recent_grads, aes(x = median, y = sharewomen, color = stem)) +
  geom_point(alpha = 0.7) +
  labs(title = "Median Income vs Proportion of Women in Majors (STEM vs Non-STEM)",
       x = "Median Income ($)",
       y = "Proportion of Women") +
  scale_color_manual(values = c("STEM" = "blue", "Non-STEM" = "red"))
```
## Further exploration
